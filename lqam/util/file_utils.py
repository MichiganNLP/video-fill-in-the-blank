...
def cached_path(
    ...
    if parsed.scheme in ("http", "https", "s3"):
        ...
        if extract_archive \
                and (is_zipfile(file_path) or tarfile.is_tarfile(file_path) or rarfile.is_rarfile(file_path)):
        ...
        if extract_archive and \
                (is_zipfile(file_path) or tarfile.is_tarfile(file_path) or rarfile.is_rarfile(file_path)):
            # This is the path the file should be extracted to.
        ...
        with FileLock(file_path + ".lock"):
            ...
            if is_zipfile(file_path):
                ...
                with rarfile.RarFile(file_path) as rar_file:
                    rar_file.extractall(extraction_path)
            else:
...
def _try_s3_prefix_sync(url: str, cache_path: str) -> bool:
    s3_resource = _get_s3_resource()
    bucket_name, s3_path = _split_s3_path(url)
    ...
    logger.info(f"Syncing from {url}")
    subprocess.run(["aws", "s3", "sync", url, cache_path], check=True)
    logger.info(f"{url} synced.")
...
def get_from_cache(url: str, cache_dir: str | Path | None = None) -> str:
    ...
    with FileLock(cache_path + ".lock"):
        if url.startswith("s3://"):
            is_a_s3_prefix_and_sync_is_successful = _try_s3_prefix_sync(url, cache_path)
        else:
            is_a_s3_prefix_and_sync_is_successful = False

        if not is_a_s3_prefix_and_sync_is_successful:
