...
class F1Scores(pl.metrics.Metric):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.add_state("score_sum", default=torch.tensor(0), dist_reduce_fx="sum")
        self.add_state("total", default=torch.tensor(0), dist_reduce_fx="sum")
    ...
    @overrides
    def update(self, preds: Sequence[str], additional_answers_list: Sequence[Sequence[str]]) -> None:  # noqa
        assert len(preds) == len(additional_answers_list)
        batch_sum = sum(compute_token_level_f1_many(pred, self._split_answers(additional_answers)) 
