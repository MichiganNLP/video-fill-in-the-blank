...
def compute_answer_level_metrics(answers_map: Mapping[str, Sequence[str]], std_answer_tokens: str,
    ...
    answer_processed_map = {worker_id: [(answer, n := normalize_answer(answer), frozenset(tokenize(n)))
                                        for answer in worker_answers]
                            for worker_id, worker_answers in answers_map.items()}

    std_answer_normalized = normalize_answer(std_answer_tokens)
    std_answer_tokens = frozenset(tokenize(std_answer_normalized))
